...
| 進捗・ナビゲーション・キャッシュ | コースとロードマップ間の進捗率整合（チャレンジ/クイズ/スキルコーディングの正解状態含む）、`from=roadmap` 引き継ぎ、前後レッスン、フォールバック、キャッシュ更新 |
| データ整合性・コードレビュー | 並び順・関連レコード・slug一意性・トランザクション、UUID/型定義/エラーハンドリング/公開条件などコードレビュー指摘の修正と別課題整理 |
...
:::message
実装内容や背景を加筆しながら改善していく予定です。内容に誤りや分かりにくい点がありましたら、ご指摘いただけるとありがたいです。
:::
